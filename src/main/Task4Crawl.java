...
public class Task4Crawl implements Runnable {
    ...
    public void run() {
        ...
        //我不换行  排版一下而已
        System.out.print("It's " + nowTime + " now");
        ...
        if (nowTime.isAfter(startTime) && nowTime.isBefore(endTime)) {
                ...
                //获取资讯对象  传递  cookie Manager
                latestInfo = new HtmlUnitDriver().getLatestInfo(cm);
                latestTime = latestInfo.getTime();

                //System.out.println(latestInfo);

                //这个记得做  要把新爬的 录进去 作为下一次 用
